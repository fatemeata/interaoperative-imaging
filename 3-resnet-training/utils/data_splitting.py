...
class DataSplitting:
    ...
    def __init__(self):
        self.wrist_data = []  # the whole dataset
    ...
    def k_fold_data(self, data):
        ...
        train, validation, test = self.create_subset(dataset=self.wrist_data, folded_dataset=folded_p_list)
        return train, validation, test

    def patient_id_list(self, dataset):
        pid_list = []
        for idx in range(len(dataset)):
            self.wrist_data.append(dataset[idx])
    ...
    def get_patient_indices(self, pid):
        ...
        """
        return [item["index"] for item in self.wrist_data if item["patient_id"] == pid]
    ...
    # def split_k_fold(self, k_fold, p_list):
    #     folded_list = []
    #     total_size = len(p_list)
    #     fraction = int(total_size / k_fold)
    #
    #     for i in range(k_fold):
    #         if i == k_fold - 1:
    #             new_list = p_list[i * fraction:]
    #         else:
    #             new_list = p_list[i * fraction: (i+1) * fraction]
    #         folded_list.append(new_list)