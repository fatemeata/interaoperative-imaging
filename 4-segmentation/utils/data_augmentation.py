...
class DataAugmentation(nn.Module):
    ...
    def _display_img_points(self, image_tensor, point_tensor):
        pass
...
class WristLineDataAugmentation(DataAugmentation):
    def __init__(self):
        super().__init__()
...
class RadiusDataAugmentation(DataAugmentation):
    def __init__(self):
        super().__init__()
        self.tensor_conversion = None
        self.image_shape = 512

    def forward(self, sample):
        img = sample['image']
        points = sample['radius_points']
        mask = sample['radius']
        # self._display_mask(img, mask)
        img = np.array(torch_transform.ToPILImage()(img)).astype(dtype="float64")
        t_img, t_points = self._geometry(img, points)
        # points_tensor = self._reshape_points_tensor(t_points)
        self.tensor_conversion = torchvision.transforms.ToTensor()
        image_tensor = self.tensor_conversion(t_img)
        
        t_mask = self.create_mask(t_points, self.img_shape)

        sample['image'] = image_tensor
        sample['radius_points'] = t_points
        sample['radius'] = t_mask

        # self._display_mask(image_tensor, t_mask)
    ...
    def create_mask(self, points, img_shape):
        ...
        rr = np.where(rr >= img_shape[0], img_shape[0]-1, rr)
        cc = np.where(cc >= img_shape[1], img_shape[1]-1, cc)
    ...
    def _geometry(self, img, points):
        rand_transform_matrix = self.geometry.perform_augmentation(self.img_shape)
        rand_transformed_image = np.float32(self.geometry.transform(item=img, matrix=rand_transform_matrix))
        rand_transformed_points = self.geometry.radius_points_translation_process(point_tensor=points,
                                                                                  matrix=rand_transform_matrix)
        return rand_transformed_image, rand_transformed_points
    
    def _display_mask(self, image_tensor, mask):
        plt.imshow(image_tensor.permute(1, 2, 0), cmap="gray")
        mask = mask.numpy()
